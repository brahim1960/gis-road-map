...
RETURNS decimal AS $$
DECLARE
  co2_factor_car decimal := 0.12;
  co2_factor_public decimal := 0.05;
  co2_saved_remote decimal := 2.5;
  total_co2 decimal := 0;
BEGIN
  SELECT COALESCE(SUM(
    CASE 
...
BEGIN
  ...
  burnout_score := (
    LEAST(avg_weekly_hours / 45.0, 1.0) * 3 +
    LEAST(overtime_frequency, 1.0) * 2 +
    (1 - LEAST(break_frequency / 3.0, 1.0)) * 2 +
    (stress_level / 10.0) * 3
  );
...
SELECT 
  ...
  CASE 
    ...
    WHEN CURRENT_TIME BETWEEN '08:00' AND '18:00' THEN 'present'::employee_status
